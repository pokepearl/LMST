...
def startmpv():
    ...
    subprocess.Popen('mpv --idle --no-audio-display --no-video --no-input-terminal --input-ipc-server=/tmp/LMST-MPV-socket', shell=True,stdin=FNULL, stderr=FNULL, stdout=FNULL)
    time.sleep(2)
...
def mpv_reset():
    ...
    subprocess.Popen(
        ...
        shell=True, stdin=FNULL, stderr=FNULL, stdout=FNULL)
    time.sleep(2)